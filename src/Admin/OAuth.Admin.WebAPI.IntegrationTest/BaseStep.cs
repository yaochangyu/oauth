...
namespace OAuth.Admin.WebAPI.IntegrationTest;

[Binding]
[CollectionDefinition("OAuth.Admin.WebAPI.IntegrationTest", DisableParallelization = true)]
public class BaseStep : Steps
{
    private static readonly List<IContainer> TestContainers = [];
    public static AdminApiTestFactory? Factory { get; private set; }
    ...
    public static async Task BeforeTestRun()
    {
        ...
        Factory = new AdminApiTestFactory();
    ...
    public void Given管理員已準備Header參數(Table table)
    {
    ...
    public void Given管理員已準備BodyJson(string json)
        => this.ScenarioContext["Body"] = json;

    [When(@"管理員發送 ""(.*)"" 請求至 ""(.*)""")]
    public async Task When管理員發送請求至(string method, string url)
    {
